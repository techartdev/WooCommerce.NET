            ...
            foreach (PropertyInfo pi in GetType().GetRuntimeProperties())
            {
                ...
                if (objValue != null && pi.GetValue(this) != null)
                {
                    if (pi.PropertyType == typeof(decimal?))
                    {
                        ...
                        var memberInfo = GetType().GetTypeInfo().BaseType;
                        if (memberInfo != null && memberInfo.FullName != null && fullName != null && (fullName.StartsWith("WooCommerceNET.WooCommerce.v1") ||
                                fullName.StartsWith("WooCommerceNET.WooCommerce.v2") ||
                                fullName.StartsWith("WooCommerceNET.WooCommerce.v3") ||
                                memberInfo.FullName.StartsWith("WooCommerceNET.WooCommerce.v1") ||
                                memberInfo.FullName.StartsWith("WooCommerceNET.WooCommerce.v2") ||
                                memberInfo.FullName.StartsWith("WooCommerceNET.WooCommerce.v3")))
                            objValue.SetValue(this, (pi.GetValue(this) as decimal?)?.ToString(CultureInfo.InvariantCulture));
                        else
