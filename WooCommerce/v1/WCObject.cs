...
namespace WooCommerce.NET.WooCommerce.v1
{
        ...
        where T10 : ShippingClass where T11 : ProductTag where T12 : TaxRate where T13 : TaxClass where T14 : Webhook
    {
        ...
        public async Task<List<T5>> GetOrderRefunds(int orderid, Dictionary<string, string> pars = null)
        {
            string json = await Api.SendHttpClientRequest("orders/" + orderid.ToString() + "/refunds", RequestMethod.GET, string.Empty, pars).ConfigureAwait(false);
